...
router.put("/like/:id", auth, async (req: CustomRequest, res: Response) => {
  try {
    ...
    if (post.likes.some((like: {user: typeof User}) => like.user.toString() === req.user.id)) {
...
router.put("/unlike/:id", auth, async (req: CustomRequest, res: Response) => {
  try {
    ...
    // Get remove index
    const removeIndex = post.likes.map((like: {user: typeof User}) => like.user.toString()).indexOf(req.params.id);
...
// @desc     Delete comment
// @access   Private
router.delete("/comment/:id/:comment_id", auth, async (req: CustomRequest, res: Response) => {
  try {
    const post = await Post.findById(req.params.id);

    // Pull out comment
    const comment = post.comments.find((comment: {id: string}) => comment.id === req.params.comment_id);
    ...
    post.comments = post.comments.filter(({id}: {id: string}) => id !== req.params.comment_id);
